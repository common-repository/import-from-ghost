...
class Ghost_Importer {
    ...
    public function runImport($importfile) {
        ...
        if (!$this->json) $this->error('Please check your export file is valid JSON and try again.');
    ...
    private function importSetting($setting) {
        ...
        switch ($setting->key){
            case "title":
                ...
                if (!$this->dryrun) update_option('blogname',$setting->value);
                break;

            case "description":
                $this->log("Setting blog description to '".$setting->value."'");
                if (!$this->dryrun) update_option('blogdescription',$setting->value);
                break;

            case "postsPerPage":
                $this->log("Setting posts-per-page to '".$setting->value."'");
                if (!$this->dryrun) update_option('posts_per_page',$setting->value);
                break;
    ...
    private function importPost($post) {
        ...
        if ($post->meta_description) $newpost['meta_input']['_yoast_wpseo_metadesc'] = $post->meta_description;
        if ($post->meta_title) $newpost['meta_input']['_yoast_wpseo_title'] = $post->meta_title;
    ...
    private function getTagsForPost($id) {
        ...
        foreach ($this->json->db[0]->data->posts_tags as $post_tag) {
            if ($post_tag->post_id !== $id) continue;
            foreach ($this->json->db[0]->data->tags as $tag) {
                if ($tag->id !== $post_tag->tag_id) continue;
                $tags[] = $tag->name;
            }
        }
        return $tags;
    ...
    private function migrateImages($postid,$content) {
        ...
        if (!$this->dryrun) $content = $post->post_content;
        // Run regex to find image URLs in content
        $matches = [];
        preg_match_all('/(?:src=")(\/content\/images\/.\S+)(?:" )/',$content,$matches, PREG_PATTERN_ORDER);
        if (!empty($matches[0])) {
          ...
          if (!$this->dryrun) wp_update_post(array('ID'=>$post->ID,'post_content'=>$content)); 
        }
    }
